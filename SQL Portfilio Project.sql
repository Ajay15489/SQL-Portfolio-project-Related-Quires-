...
select * from orders_data;
 ...
 from orders_data order by profit_ratio
 ...
 select * from orders_data
 where quantity in (3,4,5,6)
 order by quantity

 select * from orders_data
 where quantity not in (3,4,5,6)
 order by quantity
...
select sum(sales) as total from orders_data
select min(sales) as total from orders_data
select max(sales) as total from orders_data
select avg(sales) as total from orders_data
...
select city,SUM(sales) as _city_sales
from orders_data
where region = 'west'
group by city
...
-- all data which is not retunrned

select *
,case when return_reason='wrong item' then 'wrong item' else return_reason end new_return
from returns_data

--when profit<0 -> loss,profit > 50 then low profit , <100 very high pofit
...
,case when profit < 0 then 'loss'
 ...
 select *
 ,LEN(customer_name) as cust_lenth
 from orders_data

 select *
 ,LEN(customer_name) as cust_lenth
 ,left(order_id,2)  as cust_lenth
 from orders_data

 select *
 ,LEN(customer_name) as cust_lenth
 ,right(order_id,2)  as cust_lenth
 from orders_data
 ...
 , REPLACE(customer_name,'c','p')
  ...
  ,datepart(year,order_date) as order_year
   ...
   select order_id,order_date
  ,datepart(month,order_date) as order_year
   from orders_data

   select order_id,order_date
  ,datepart(week,order_date) as order_year
   from orders_data

   select order_id,order_date
  ,datepart(QUARTER,order_date) as order_year
   from orders_data
  ...
  from orders_data
   ...
   select order_id,order_date,ship_date
  ,datediff(year,order_date,ship_date) as lead_days
